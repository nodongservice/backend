...
public class UserWithdrawalFinalizeScheduler {
    ...
    // 문자열 파싱/빈 참조 오류를 피하기 위해 스케줄러 전용 밀리초 설정을 사용한다.
    @Scheduled(
            fixedDelayString = "${bridgework.auth.withdrawal-finalize-interval-ms:3600000}",
            initialDelayString = "${bridgework.auth.withdrawal-finalize-interval-ms:3600000}"
    )
