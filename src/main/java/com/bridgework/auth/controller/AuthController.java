...
public class AuthController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "로그인 성공",
                    content = @Content(examples = {
                            @ExampleObject(
                                    name = "기존 회원 응답 예시",
                                    value = "{\"code\":\"SUCCESS\",\"message\":\"요청이 성공했습니다.\",\"result\":{\"signupRequired\":false,\"signupToken\":null,\"provider\":\"KAKAO\",\"email\":\"user@example.com\",\"name\":\"홍길동\",\"accountStatus\":\"ACTIVE\",\"withdrawalDeadlineAt\":null,\"withdrawalCancelToken\":null,\"tokenPair\":{\"accessToken\":\"<ACCESS_TOKEN>\",\"tokenType\":\"Bearer\",\"accessTokenExpiresAt\":\"2026-05-08T08:00:00Z\",\"refreshTokenExpiresAt\":\"2026-05-22T08:00:00Z\"}}}"
                            ),
                            @ExampleObject(
                                    name = "최초 회원 응답 예시",
    ...
    public ResponseEntity<com.bridgework.common.dto.ApiResponse<SocialLoginResponseDto>> socialLogin(@Valid @RequestBody SocialLoginRequestDto request) {
        ...
        if (response.tokenPair() != null) {
            builder.header(HttpHeaders.SET_COOKIE, refreshTokenCookieService.createHeader(response.tokenPair().refreshToken()));
        }
        return builder.body(com.bridgework.common.dto.ApiResponse.success(response));
    }
    ...
    @ApiResponse(responseCode = "200", description = "가입 완료 및 토큰 발급",
            content = @Content(examples = @ExampleObject(
                    value = "{\"code\":\"SUCCESS\",\"message\":\"요청이 성공했습니다.\",\"result\":{\"accessToken\":\"<ACCESS_TOKEN>\",\"tokenType\":\"Bearer\",\"accessTokenExpiresAt\":\"2026-05-08T08:00:00Z\",\"refreshTokenExpiresAt\":\"2026-05-22T08:00:00Z\"}}"
            )))
    public ResponseEntity<com.bridgework.common.dto.ApiResponse<TokenPairResponseDto>> completeSignup(@Valid @RequestBody SignupCompleteRequestDto request) {
        return tokenResponse(authService.completeSignup(request));
    }

    @PostMapping("/token/refresh")
    @Operation(summary = "토큰 재발급", description = "HttpOnly 리프레시 쿠키 검증 후 액세스 토큰과 리프레시 쿠키를 재발급한다.")
    @ApiResponse(responseCode = "200", description = "토큰 재발급 성공",
            content = @Content(examples = @ExampleObject(
                    value = "{\"code\":\"SUCCESS\",\"message\":\"요청이 성공했습니다.\",\"result\":{\"accessToken\":\"<NEW_ACCESS_TOKEN>\",\"tokenType\":\"Bearer\",\"accessTokenExpiresAt\":\"2026-05-08T08:15:00Z\",\"refreshTokenExpiresAt\":\"2026-05-22T08:15:00Z\"}}"
            )))
    public ResponseEntity<com.bridgework.common.dto.ApiResponse<TokenPairResponseDto>> refreshToken(
    ...
    @Operation(summary = "로그아웃", description = "HttpOnly 리프레시 쿠키의 세션과 해당 세션의 액세스 토큰을 즉시 무효화한다.")
    @ApiResponse(responseCode = "200", description = "로그아웃 성공")
    public ResponseEntity<com.bridgework.common.dto.ApiResponse<Void>> logout(
            HttpServletRequest servletRequest,
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE, refreshTokenCookieService.clearHeader())
                .body(com.bridgework.common.dto.ApiResponse.success(null));
    }
    ...
    public ResponseEntity<com.bridgework.common.dto.ApiResponse<TokenPairResponseDto>> cancelWithdraw(@Valid @RequestBody WithdrawCancelRequestDto request) {
        return tokenResponse(authService.cancelWithdrawal(request.withdrawalCancelToken()));
    }

    private ResponseEntity<com.bridgework.common.dto.ApiResponse<TokenPairResponseDto>> tokenResponse(
            TokenPairResponseDto tokenPair
    ) {
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE, refreshTokenCookieService.createHeader(tokenPair.refreshToken()))
                .body(com.bridgework.common.dto.ApiResponse.success(tokenPair));
    }
