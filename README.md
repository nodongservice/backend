...
- `aiEnabled=true`: 프론트가 `limit`/`offset`을 보내지 않으면 백엔드가 현재 유효 공고 전체 개수로 정규화하고 `offset=0`으로 계산합니다.
- `aiEnabled=false`: `limit`은 1~100 범위이며 `offset`은 페이지네이션 시작 위치입니다.
...
- AI ON 전체 공고 계산은 내부적으로 1개 단위 FastAPI 요청을 순차 실행하고, 각 항목 완료 시 `PROCESSING` envelope의 `result`에 누적 부분 결과를 저장합니다.
...
- 가입 완료(온보딩)는 프로필 필수 항목만 입력받아 기본 프로필 1개를 생성한다.
...
- `disabilitySeverity`: 중증, 중등도, 경증
- `workAvailability`: 즉시, 2주 이내, 1개월 이내, 협의 가능
- `workTypes`: 정규직, 계약직, 무기계약직, 시간제, 일용직, 인턴, 파견/용역, 재택/원격
...
- `ACCESSIBILITY_FEEDBACK_DISCORD_WEBHOOK_URL`
- `BRIDGEWORK_AUTH_JWT_SECRET`
- `KAKAO_CLIENT_SECRET`
- `NAVER_CLIENT_SECRET`
- `BRIDGEWORK_FASTAPI_HEALTH_URL` (선택, 미지정 시 `http://bridgework-aiserver-blue:8000/health,http://bridgework-aiserver-green:8000/health`)
- `BRIDGEWORK_RECOMMEND_FASTAPI_BASE_URL` (선택, 미지정 시 `http://bridgework-aiserver:8000`)
